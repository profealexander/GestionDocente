<script lang="ts">
	...
	async function selectGrade(grade: Grade) {
		...
		try {
			...
			// Inicializar todas las celdas como null (sin marcar)
			for (const s of students)
				for (const h of homeworks)
					cells[`${s.id}:${h.id}`] = null;

			// Cargar submissions existentes para pre-poblar la tabla
			const subFetches = homeworks.map(h =>
				api.get<{student_id:number, status:string}[]>(`/homework/${h.id}/submissions`)
					.then(rows => rows.forEach(r => {
						const key = `${r.student_id}:${h.id}`;
						if (r.status === 'delivered') cells[key] = true;
						else if (r.status === 'missing') cells[key] = false;
					}))
					.catch(() => {}) // ignorar si falla individual
			);
		...
		try {
			const requests = homeworks.map(hw => {
				const records = students
					.filter(s => cells[`${s.id}:${hw.id}`] !== null)
					.map(s => ({
						student_id: s.id,
						status: cells[`${s.id}:${hw.id}`] === true ? 'delivered' : 'missing',
					}));
				if (records.length === 0) return Promise.resolve();
				return api.post(`/homework/${hw.id}/report`, records);
			});
