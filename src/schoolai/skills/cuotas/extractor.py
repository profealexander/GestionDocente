...
_CREATE_RE = re.compile(
    r"\b(nueva?\s+actividad|crear\s+actividad|agregar\s+actividad|nueva\s+cuota|crear\s+cuota)\b",
    re.IGNORECASE,
)
_PAGO_RE = re.compile(
...
_LIST_RE = re.compile(
    r"\b(listar\s+actividades?|ver\s+actividades?|actividades?\s+activas?|"
    r"qué\s+actividades?|que\s+actividades?)\b",
    re.IGNORECASE,
)
...
_NOMBRE_AFTER_RE  = re.compile(
    r"\b(?:actividad|cuota)\s+(?:de\s+)?([A-ZÁÉÍÓÚÑ][a-záéíóúñ\s]{2,60}?)(?=\s*\$|\s*\d|\s*para|\s*de\s+\d|$)",
    re.IGNORECASE,
)
...
_GENERIC_WORDS = frozenset(["cuotas", "cuota", "actividades", "actividad", "pagos", "pago", "reporte"])
...
@dataclass
class CuotaExtract:
    action: Literal["create", "pago", "query", "export", "list", "add_students"]
