"""Extracción de entidades usando LLM (GLM / compatible con OpenAI)."""
...
async def extract(text: str, history: list[dict] | None = None) -> ExtractionResult | None:
    """Extrae intent y entidades del mensaje usando LLM. Retorna None si falla."""
    provider, model = parse_model(settings.llm_extractor)
    try:
        client = get_client(provider, timeout=30.0)
    except ValueError as e:
        logger.warning(f"[extract_llm] {e}")
    ...
    try:
        ...
        if finish == "length":
            logger.warning("[extract_llm] JSON truncado por max_tokens")

        match = _JSON_RE.search(content)
        if not match:
            logger.warning(f"[extract_llm] no JSON (attempt 1) finish={finish!r} — retrying...")
        ...
        if not match:
            logger.error(f"[extract_llm] no JSON after retry finish={finish!r}")
            return None

        raw = json.loads(match.group())
        result = _build_result(raw)
        logger.debug(f"[extract_llm] raw={match.group()}")
        logger.info(f"[extract_llm] intent={result.intent} complete={getattr(result.data, 'complete', True)}")
        return result

    except Exception as e:
        logger.error(f"[extract_llm] error: {e}")
        return None
