...
class ActividadParticipante(Base):
    ...
    __table_args__ = (UniqueConstraint("actividad_id", "student_id", name="uq_actividad_student"),)
    ...
    def __repr__(self) -> str:
        return (
            f"<ActividadParticipante id={self.id} "
            f"student_id={self.student_id} total={self.total_pagado}>"
        )
...
class ActividadPago(Base):
    ...
    participante_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("actividad_participantes.id"), nullable=False,
    )
    monto: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)
    ...
    def __repr__(self) -> str:
        return (
            f"<ActividadPago id={self.id} "
            f"participante_id={self.participante_id} monto={self.monto}>"
        )
