...
class Teacher(Base):
    ...
    person_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("people.id"), nullable=False, unique=True,
    )
    ...
    positions: Mapped[list["TeacherPosition"]] = relationship(
        "TeacherPosition", back_populates="teacher", cascade="all, delete-orphan",
    )  # noqa: F821
...
class Schedule(Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<Schedule teacher={self.teacher_id} day={self.day_of_week} period={self.period_num}>"
        )
...
class TeacherPosition(Base):
    ...
    )  # tutor|jefe_area|jefe_subnivel|comision|cargo
    grade_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("grades.id"), nullable=True)
    subnivel: Mapped[str | None] = mapped_column(
        String(20), nullable=True,
    )  # inicial|basica|media|bachillerato
    area: Mapped[str | None] = mapped_column(String(100), nullable=True)
    detail: Mapped[str | None] = mapped_column(
        String(200), nullable=True,
    )  # cargo value or comision name
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
